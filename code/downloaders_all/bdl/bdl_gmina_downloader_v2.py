...
CURATED_VARIABLES = {
    # -----------------------------------------------------------------------
    # Variable IDs verified against bdl_variable_catalogue_gmina.csv
    # generated by --discover on 2026-03-09.
    #
    # For finance variables, we use "gminas include cities with powiat
    # status" which covers ALL gmina-level units in a single variable.
    # -----------------------------------------------------------------------

    # --- DEMOGRAPHICS ---
    "population_total": {
        "var_id": 72305,
        "description": "Total population (both sexes, all ages)",
        # subgroup: Population by sex and age group | var_name: total | var_name_full: total
    },
    "births": {
        "var_id": 59,
        "description": "Live births — total",
        # subgroup: Vital statistics by sex | var_name_full: total
    },
    "deaths": {
        "var_id": 65,
        "description": "Deaths, grand total",
        # subgroup: Vital statistics by sex | var_name_full: total
    },
    "migration_net": {
        "var_id": 1365234,
        "description": "Total net migration (internal + international)",
        # subgroup: Gmina migrations for permanent residence … in internal and international movement
    },
    # --- LABOUR MARKET ---
    "registered_unemployed": {
        "var_id": 10514,
        "description": "Registered unemployed persons — total",
        # subgroup: Registered unemployed persons by sex in gminas
    },
    "unemployment_share": {
        "var_id": 79214,
        "description": "Share of registered unemployed in working-age population (%)",
        # subgroup: Share of the registered unemployed persons in the population
    },
    # --- LOCAL GOVERNMENT FINANCE (all: gminas incl. cities with powiat status) ---
    "gmina_revenue_total": {
        "var_id": 76037,
        "description": "Total gmina budget revenue (PLN)",
        # subgroup: Revenue, total
    },
    "gmina_own_revenue": {
        "var_id": 76070,
        "description": "Gmina own revenue — grand total (PLN)",
        # subgroup: Own revenue of gmina budgets
    },
    "pit_share": {
        "var_id": 76046,
        "description": "Gmina share of personal income tax (PIT) (PLN)",
        # subgroup: Own revenue of gmina budgets
    },
    "cit_share": {
        "var_id": 76043,
        "description": "Gmina share of corporate income tax (CIT) (PLN)",
        # subgroup: Own revenue of gmina budgets
    },
    "investment_expenditure": {
        "var_id": 76450,
        "description": "Investment property expenditure (PLN)",
        # subgroup: Budget expenditure
    },
    # --- EDUCATION ---
    "primary_school_pupils": {
        "var_id": 151867,
        "description": "Pupils in primary schools — total",
        # subgroup: Primary schools, total
    },
    # --- INFRASTRUCTURE ---
    "water_supply_network_km": {
        "var_id": 489,
        "description": "Length of working water distribution network (km)",
        # subgroup: Water supply systems
    },
    "sewage_network_km": {
        "var_id": 494,
        "description": "Length of active sewage network (km)",
        # subgroup: Sewage system
    },
    # --- BUSINESS / ECONOMY ---
    "registered_entities": {
        "var_id": 194813,
        "description": "Entities in REGON register — total",
        # subgroup: Entities of the national economy entered in the REGON register
    },
    "new_entities_registered": {
        "var_id": 458178,
        "description": "Newly registered entities — total",
        # subgroup: Entities newly registered by groups of kind activity
    },
    # --- HOUSING ---
    "dwellings_completed": {
        "var_id": 748601,
        "description": "Dwellings completed — total",
        # subgroup: Dwellings completed
    },
}
...
def download_curated_variables(client, output_dir, start_year, end_year):
    """
    Download data for curated variables using verified var_ids.
    No search needed — IDs come from the discovery catalogue.
    """
    log.info("=" * 70)
    log.info("PHASE 2: DOWNLOADING CURATED VARIABLES FOR ALL GMINAS")
    log.info(f"Years: {start_year}–{end_year}")
    log.info(f"Variables: {len(CURATED_VARIABLES)}")
    log.info("=" * 70)
    ...
    for key, spec in CURATED_VARIABLES.items():
        var_id = str(spec["var_id"])
        description = spec["description"]

        log.info(f"\n--- {key}: {description} (var_id={var_id}) ---")

        # Download the data
        rows = download_variable_data(
            client, var_id, description, years, output_dir
        )
